...
void Offsets::OffsetUpdate()
{
	...
	offsets.dwEntityList = FindPattern(offsets.client, bytes, "BB ? ? ? ? 83 FF 01 0F 8C ? ? ? ? 3B F8", 0x1, 0x0) - offsets.clientBase; //0x1 is the offset, there is no extra
	...
	offsets.dwForceAttack = FindPattern(offsets.client, bytes, "89 0D ? ? ? ? 8B 0D ? ? ? ? 8B F2 8B C1 83 CE 04", 0x2, 0x0) - offsets.clientBase;
